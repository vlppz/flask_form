...
def start():
    ...
    rooms.update(
        {last_id: {'words': [random.choice(words2)], 'users': [], 'host': request.args["name"]}})
...
def game(code):
    ...
    if request.args.get('message', False):
        ...
        if ((word[0].lower() == lastword[-1].lower()) and (word.lower() not in rooms[code]["words"])):
            rooms[code]["words"].append(
                request.args.get('message', False).lower())
